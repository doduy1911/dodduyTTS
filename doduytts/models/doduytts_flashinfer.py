...
"""FlashInfer-accelerated iterative decoding for DoDuyTTS.
...
- Attention: registered as a custom HF attention implementation
  ("doduytts_fi") via AttentionInterface; reads the wrapper planned
...
Usage:
    from doduytts_flashinfer import apply_flashinfer
...
AttentionInterface.register("doduytts_fi", _flashinfer_attention)
...
def _generate_iterative_packed(
    ...
    """Packed-sequence rewrite of DoDuyTTS._generate_iterative.
...
def apply_flashinfer(
    ...
    """Patch an DoDuyTTS instance to use flashinfer packed attention."""
    model.llm.set_attn_implementation("doduytts_fi")
    if fuse_rmsnorm:
